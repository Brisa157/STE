    ...
    <body>
        ...
        <div class="content-page-container full-reset custom-scroll-containers">
            ...
            <div class="container-fluid">
                <a class="btn btn-default" href="niveles_add_edit.php?id=0">Nuevo</a>
                <br><br>
                <div class="table-responsive">
                    <table id="tblData"  class="table table-striped table-bordered" style="width:100%">
                        ...
                        <tbody>
                            <?php
                            $query = "SELECT * FROM alumno INNER JOIN ingles ON alumno.id_alumno = ingles.id_alumno NATURAL JOIN carrera";
                            $result = mysqli_query($con, $query);
                            if ($result) {
                                foreach ($result as $data) {
                                    $data['acciones'] = '<a class="btn btn-info" href="niveles_add_edit.php?id='.$data['id_ingles'].'">Actualizar</a>';
                                    $numero_control = $data['numero_control'];
                                    $nombre_completo = $data['nombre_completo'];
                                    $semestre = $data['semestre'];
                                    $carrera = $data['carrera'];
                                    $nivel = $data['nivel'];
                                    $calificacion_final = $data['calificacion_final'] == ''? 'Pendiente':$data['calificacion_final'];
